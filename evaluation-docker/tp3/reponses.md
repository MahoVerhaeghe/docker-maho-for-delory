...
**Question :** Quel est l'avantage d'un réseau personnalisé par rapport au réseau par défaut ?
    ...
    - DNS personnalisé (plus simple pour les IP par ex)
    - Sécurité des ports (pas/peu exposé) et des communications grâce au bridge
...
```bash
docker exec -it eval-redis redis-cli monitor
```
- Que se passe-t-il si vous arrêtez le service Redis ?
    On ne passe pas par Redis mais vers la DB directement (ou crash ?)
